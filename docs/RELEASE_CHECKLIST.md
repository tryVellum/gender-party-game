...
Проверьте ссылки и номер версии `1.2.0`.
...
```powershell
git commit -m "Prepare Windows release 1.2.0"
...
```powershell
git tag -a v1.2.0 -m "Gender Party Game 1.2.0"
...
3. откройте запуск для тега `v1.2.0`;
...
В Release `v1.2.0` должны находиться:

```text
GenderPartyGame-Setup-1.2.0.exe
GenderPartyGame-Portable-1.2.0.zip
```
